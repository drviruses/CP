...
void virus(){
       ...
       cin>>num>>a>>b>>c;
       vector<ll> div;
       div.push_back(a); div.push_back(b); div.push_back(c);
       vector<ll> arr;
       arr.push_back(num);
       for(auto i=0; i<3;){
           if(arr.back()%div[i] == 0){
               ll t = arr.back()/div[i];
               string t_seq = to_string(t);
               arr.push_back(t_seq[0] - '0');
           }
           else{
               ll r = (arr.back()%div[i]);
               string divr = to_string(div[i]);
               ll t = (r*(pow(10,(ll)divr.size())))/div[i];
               string t_seq = to_string(t);
               arr.push_back(t_seq[0] - '0');
           }
           if(i == 2) i=0; else i+=1;
       ...
       while(q--){
           ll indx;
           cin>>indx;
           if(indx >= arr.size()) cout<<0<<endl;
           else cout<<arr[indx]<<endl;
       }
}
