...
void virus(){
       ...
       vector<pair<ll,ll>> pr(cnt.begin(),cnt.end());
       sort(pr.begin(),pr.end(),mysort);
       ll fir = pr[0].second, cnt_s = 0;
       for(auto i:pr){
           if(i.second == fir) cnt_s++;
       }
       ll left = len - (cnt_s * fir);
       if(fir != 1) cout<<cnt_s + ((left)/(fir-1)) - 1<<endl;
       else cout<<cnt_s-1<<endl;
}
