...
void virus(){
       ...
       ll range = arr.back() - arr.front();
       while(range){
           ll t_cow = cow - 2;
           ll pos = lower_bound(arr.begin(),arr.end(),arr.front()+range) - arr.begin();
           cout<<"Rang: "<<range<<endl;
           while(t_cow){
               pos = lower_bound(arr.begin(),arr.end(),arr[pos-1]+range) - arr.begin();
               cout<<pos<<endl;
               if(pos == len) break;
               else t_cow -= 1;
           }
