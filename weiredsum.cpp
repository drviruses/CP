...
void virus(){
        ...
        vector<ll> moda(m-1),pr;
        iota(moda.begin(),moda.end(),1);
        moda.insert(moda.end(),0);
        ll times = ceil((ld)k/(ld)m);
        for(auto i=0; i<times; i++){
            for(auto j=0; j<m; j++) pr.push_back(moda[j]);
        }

        ll ans = LLONG_MIN;
        ull cntr = pow(2LL,len);
        for(auto i=1; i<cntr; i++){
            vector<ll> temp;
            ...
            if(temp.size() == k){
                ll sum = 0;
                for(auto it=0; it<k; it++) sum +=(temp[it]*pr[it]);
                ans = max(ans,sum);
            }
        }
        cout<<ans;
