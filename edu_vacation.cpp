...
void virus(){
          ...
          for(auto i=1; i<len; i++){
              arr[i][0] = max(arr[i][0] + arr[i-1][1], arr[i][0] + arr[i-1][2]);
              arr[i][1] = max(arr[i][1] + arr[i-1][0], arr[i][1] + arr[i-1][2]);
              arr[i][2] = max(arr[i][2] + arr[i-1][1], arr[i][2] + arr[i-1][0]);
          }
          cout<<*max_element(arr[len-1].begin(),arr[len-1].end())<<endl;
}
