...
void virus(){
        ...
        for(auto &i:A) cin>>i;
        for(auto &i:B) cin>>i;
        for(auto i=0; i<len; i++) day.push_back({A[i],B[i]});
        sort(day.begin(),day.end(),mysort);
        ll sum = 0, j = k;
        for(auto i:day) if(k) sum += i.first, k--;
        for(; j<len; j++){
            if(day[j].second >= day[j].first) sum += day[j].first;
            else break;
        }
        for(auto i=j; i<len; i++) sum += day[i].second;
        cout<<sum;
}
