...
void virus(){
       ...
       for(auto i=1; i<len; i++){
           ...
           else { alt.push_back(cnt); cnt = 1; }
       }
       alt.push_back(cnt);
       ll sum = 0;
       bool pm = true;
       for(auto i:alt){
           if(pm) sum += (i*(-1)), pm = false;
           else sum += i, pm = true;
       }
       if(abs(sum) == 1) cout<<"Case #"<<google_itr<<": "<<"Y\n";
       else cout<<"Case #"<<google_itr<<": "<<"N\n";
       google_itr++;
}
...
int32_t main(){
    ...
    #ifndef ONLINE_JUDGE
    freopen("inputBB.txt","r",stdin);
    freopen("outputBB.txt","w",stdout);
    #endif
