...
void virus(){
        ...
        ll ans = (ll)seq.size() - arr[0].second, pr_min = LLONG_MAX;
        for(auto i:"0123456789"){
            for(auto j:"0123456789"){
                if(i != j){
                    string temp;
                    temp += i,temp += j;
                    ll cnt = 0;
                    bool enc = false;
                    for(auto k=0; k<(ll)seq.size(); k++){
                        if(!enc and seq[k] == temp[0]) cnt++, enc = true;
                        if(enc and seq[k] == temp[1]) cnt++, enc = false;
                    }
                    cnt = (cnt&1)? --cnt : cnt;
                    pr_min = min(pr_min,(ll)seq.size() - cnt);
                }
            }
        }
        cout<<min(ans,pr_min)<<endl;
}
