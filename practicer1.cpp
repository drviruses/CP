...
void virus(){
       ...
       for(auto i=0; i<len; i++){
           ll k=0,j=0;
           for(j=i+1; j<len; j++,k++){
               if(seq[k] == seq[j]) continue; else break;
           }
           if(j == len and k > 0){
               string temp = seq.substr(k++);
               if((ll)temp.size() > ans) ans = temp.size(), ans_t += temp;
               break;           
           }
       }
       if(!ans_t.size()) ans_t = seq;
       for(auto i=0; i<k-1; i++) seq += ans_t;
       cout<<seq<<endl;
}
