...
char winchar(char temp){
    if(temp == 'R') return 'P';
    else if(temp == 'P') return 'S';
    return 'R';
}

bool mysort(pair<char,ll> A, pair<char,ll> B){
...
void virus(){
       ...
       string ans = "";
       vector<pair<char,ll>> fr(cnt.begin(),cnt.end());
       sort(fr.begin(),fr.end(),mysort);
       for(auto i:fr){
           for(auto j=0; j<i.second; j++){
                char temp = i.first;
                if(temp == 'R') ans += (char)('P');
                else if(temp == 'P') ans += (char)('S');
                else ans += char('R');
           }
       }
       cout<<ans<<endl;
