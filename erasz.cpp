...
void virus(){
       ...
       cin>>seq;
       deque<ll> one_indx;
       for(auto i=0; i<(ll)seq.size(); i++) if(seq[i] == '1') one_indx.push_back(i);
       if(!one_indx.size()) cout<<"0\n";
       else cout<<count(seq.begin()+one_indx.front(),seq.begin()+one_indx.back(),'0')<<endl;
}
