...
void mybfs(vector<ll> arr){
    queue<pair<vector<ll>,ll>> q;
    q.push({arr,0});
    while(!q.empty()){
        auto curr = q.front();
        q.pop();
        if(is_sorted(curr.first.begin(),curr.first.end())){
            ans = curr.second;
            return;
        }
        for(auto i=2; i<=(ll)arr.size(); i++){
            vector<ll> rec(curr.first);
            reverse(rec.begin(),rec.begin()+i);
            if(uniq.find(rec) == uniq.end()){
                uniq.insert(rec);
                q.push({rec,curr.second+1});
            }
        }
    }
}
